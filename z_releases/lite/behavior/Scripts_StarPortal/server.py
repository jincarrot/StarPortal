...
def onInteractCorePortal(player, portal):
    ...
    if player.mainHand and player.mainHand.typeId == "minecraft:nether_star":
        player.sendMessage("完整版内容，体验版暂未开放！")
        return
        data = getPortalData()
        ...
        if arg.source.asPlayer().playerPermissionLevel == 2:
            ...
            def delete():
                ...
                def deletePortal(portalData):
                    ...
                    nodes = portalData
                    while nodes:
                        arg.source.sendMessage("星塔[%s]已删除" % nodes['name'])
                        portal = world.getEntity(nodes['entityId'])
                        if portal and portal.isValid:
                            clearPortalLight(portal)
                            portal.remove()
                        else:
                            portalsNeedDel.append(nodes['entityId'])
                        nodes = portalData['nodes']
            ...
            def developer():
                form.close()
                showAll = Observable.create(False, {"clientWritable": True})
                output = Observable.create("", {"clientWritable": True})
                def onShowAllChange(new_value):
                    value = """stream.getvalue()""" # type: str
                    if not new_value:
                        info = ""
                        ...
                        for v in values:
                            info += v + "\n"
                        output.setData(info)
                showAll.subscribe(onShowAllChange)
                devForm = CustomForm.create(arg.source, "查看数据")
                devForm.toggle("完整数据", showAll)
                # devForm.label(json.dumps(getPortalData(), indent=4, ensure_ascii=False))
                onShowAllChange(showAll.getData())
                devForm.label(output)
                ui = MoreUI.create(arg.source, {"column": [1, 10, 1], "row": [1, 10, 1]})
                ui.addForm(devForm, {"position": [1, 1], "size": [1, 1]})
                ui.show()
            ...
            def onEnableRecipesChange(new_value):
                world.setDynamicProperty("star_portal.enableRecipes", new_value)
                if new_value:
                    core_item = {
            ...
            arg.source.sendMessage("§c您没有权限打开星塔管理菜单！")
...
system.afterEvents.clientEventRecieve.subscribe("openStarPortalManager", openStarPortalManager)

def initialRecipes():
    if world.getDynamicProperty("star_portal.enableRecipes"):
        import mod.server.extraServerApi as serverApi
        comp = serverApi.GetEngineCompFactory().CreateRecipe(serverApi.GetLevelId())
        core_item = {
            "minecraft:recipe_shaped": {
