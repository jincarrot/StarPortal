...
class ItemStack(object):
    ...
    def __init__(self, itemType, amount, data={}):
        # type: (str, int, dict) -> None
        self.__typeId = data.get("newItemName", itemType)
        self.amount = data.get("count", amount)
    ...
    def __generateComponents(self):
        ...
        itemData = SComp.CreateItem(serverApi.GetLevelId()).GetItemBasicInfo(typeId) or {}
        if itemData.get("enchant_slot_type", 0):
            self.__components["minecraft:enchantable"] = ItemEnchantableComponent({"slots": itemData.get("enchant_slot_type")})
        if itemData.get("maxDurability", 0):
            self.__components['minecraft:durability'] = ItemDurabilityComponent({"maxDurability": itemData['maxDurability']})

    def __generateData(self):
        if not self.__data:
            return
        itemDict = self.__data
        if 'userData' in itemDict and itemDict['userData']:
            userData = itemDict['userData']
            if 'minecraft:keep_on_death' in userData:
                self.keepOnDeath = userData['minecraft:keep_on_death']['__value__'] == 1
        ...
        if 'extraId' in itemDict and itemDict['extraId'] and itemDict['extraId'][0] == '{' and itemDict['extraId'][-1] == '}':
            try:
                self.setDynamicProperties(ast.literal_eval(itemDict['extraId']))
            finally:
                pass
        if self.hasComponent("minecraft:enchantable"):
            ...
            self.getComponent("minecraft:enchantable").asEnchantableComponent().addEnchantments(enchantList)
        if self.hasComponent("minecraft:durability"):
            remain = itemDict['durability']
            self.getComponent("minecraft:durability").asDurabilityComponent().remain = remain
    ...
    def getItemDict(self):
        ...
        for componentId in self.__components:
            ...
            if componentId == 'minecraft:enchantable':
                enchantmentList = self.__components['minecraft:enchantable'].asEnchantableComponent().getEnchantments()
                enchantData = []
                customEnchantData = []
                ...
                data['durability'] = self.__components['minecraft:durability'].asDurabilityComponent().remain
        if not self.__data.get('userData'):
            self.__data['userData'] = {}
        return update(self.__data, data)
...
def createItemStack(itemDict):
    ...
    if not itemDict:
        return None
    return ItemStack(itemDict['newItemName'], itemDict['count'], itemDict)
