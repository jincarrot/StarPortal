...
class EventListener(object):
    ...
    def __init__(self, eventName, callback, options=None, detectFunc=None, valueName=None, wrapper=None, namespace=clientApi.GetEngineNamespace(), systemName=clientApi.GetEngineSystemName()):
        ...
        CComp.CreateItem(clientApi.GetLevelId()).GetUserDataInEvent(eventName)
        event = listenerManager.get(eventName)
        if event.empty:
            client.ListenForEvent(namespace, systemName, eventName, event, event.listen)
