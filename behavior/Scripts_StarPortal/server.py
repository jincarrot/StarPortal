    ...
    if player.isSneaking:
        ...
        class temp:
            def __init__(self, data):
                self.data = data
        openStarPortalManager(temp({"playerId": player.id, "portalId": portalData['id']}))
        return
...
def onInteractTempPortal(player, portal):
    ...
    targetLocation = (targetLocation[0] + 0.5, targetLocation[1] + 8, targetLocation[2] + 0.5, targetLocation[3])
    def doTeleport():
        player.teleport(targetLocation, {"dimension": world.getDimension(targetLocation[3])})
        player.addEffect("slow_falling", 140, {"showParticle": False})
...
def openStarPortalManager(arg):
    ...
    for entityId in player.getDynamicProperty("elspirit:interactions") or []:
        ...
        if entity:
            ...
            if not nodeData:
                continue
            teleport.button("%s\n%s" % (nodeData['name'], nodeData['location'][:3:]), lambda: onTeleport(player, world.getEntity(entityId)))
    # MoreUI
    ui = MoreUI.create(player, {"column": [1, 2, 2, 1], "row": [1, 6, 1]})
